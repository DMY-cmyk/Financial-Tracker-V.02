...
export function createLiabilityRepository() {
  return {
    async findAll(): Promise<Liability[]> {
      ...
      const result = await db.query<LiabilityRow>('SELECT * FROM liabilities ORDER BY amount DESC');
      return result.rows.map(rowToLiability);
    },

    async findById(id: string): Promise<Liability | undefined> {
      const db = await getDb();
      const result = await db.query<LiabilityRow>('SELECT * FROM liabilities WHERE id = ?', [id]);
    ...
    async create(data: { name: string; amount: number; category: string }): Promise<Liability> {
      const id = nanoid();
      const db = await getDb();
      await db.query('INSERT INTO liabilities (id, name, amount, category) VALUES (?, ?, ?, ?)', [
        id,
        data.name,
        data.amount,
        data.category,
    ...
    async update(
      ...
      const db = await getDb();
      const existing = await db.query<LiabilityRow>('SELECT * FROM liabilities WHERE id = ?', [id]);
